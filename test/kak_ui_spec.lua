...
describe('ndjson framing', function()
  ...
  --- Drive `_ndjson_feed` directly. `exec_lua` returns both the parsed
  --- line list and the trailing partial buffer, so we can chain feeds.
  local function feed(chunk)
    return exec_lua(function(c)
  ...
  before_each(function()
    exec_lua(function() vim.g._kak_test_buf = '' end)
  end)
...
describe('input key translation', function()
  ...
  it('raw_to_kak with on_key-style typed string', function()
    local r = exec_lua(function() return require('kak.ui.input').raw_to_kak('<CR>j') end)
    eq(2, #r)
    eq('<ret>', r[1])
    eq('j', r[2])
  end)
  ...
  it('raw_to_kak converts <C-H>', function()
    local r = exec_lua(function() return require('kak.ui.input').raw_to_kak('<C-H>') end)
    eq(1, #r)
    eq('<c-h>', r[1])
  end)

  it('from_on_key translates raw <Esc> byte to <esc>', function()
    local r = exec_lua(function() return require('kak.ui.input').from_on_key('\27') end)
    eq(1, #r)
    eq('<esc>', r[1])
  end)

  it('from_on_key translates raw <CR> byte to <ret>', function()
    local r = exec_lua(function() return require('kak.ui.input').from_on_key('\r') end)
    eq(1, #r)
    eq('<ret>', r[1])
  end)

  it('from_on_key translates raw <Tab> byte to <tab>', function()
    local r = exec_lua(function() return require('kak.ui.input').from_on_key('\t') end)
    eq(1, #r)
    eq('<tab>', r[1])
  end)
  ...
  it('from_on_key translates raw Ctrl-A (<1>) to <c-a>', function()
    local r = exec_lua(function() return require('kak.ui.input').from_on_key('\x01') end)
    eq(1, #r)
    eq('<c-a>', r[1])
  end)
...
describe('end-to-end rpc over uv pipe', function()
  ...
  it('round-trips a notification from fake-server', function()
    local fake_path = '/tmp/kak-test-'
      .. tostring(os.time())
      .. '-'
      .. tostring(math.random(1, 1e9))
      .. '.sh'
    local f = assert(io.open(fake_path, 'w'))
    f:write([[
    ...
    eq(true, ok)
    eq('table', type(recv))
    if type(recv) == 'table' then eq('bar', recv[1] and recv[1].foo) end
  end)
end)
